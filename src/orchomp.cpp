/** \file orchomp.cpp
 * \brief orchomp entry point from OpenRAVE.
 * \author Christopher Dellin
 * \edited by Temple Price for use with chomp-multigrid
 * \date 2012
 */
...
{
   return OpenRAVE::BaseXMLReaderPtr(new orchomp::kdata_parser(boost::shared_ptr<orchomp::kdata>(),atts));
}
...
{ 
   ...
   if(!reg_reader)
      reg_reader = OpenRAVE::RaveRegisterXMLReader(OpenRAVE::PT_KinBody,"orchomp",rdata_parser_maker);
...
{
   if((type == OpenRAVE::PT_Module)&&(interfacename == "orchomp"))
      return OpenRAVE::InterfaceBasePtr(new orchomp::mod(penv));
